...
type Prefix struct {
	...
	Title      string    `orm:"column(title);size(32);type(char);default();description(前缀备注)" json:"title" form:"title" validate:"required" label:"前缀备注"`
	Tags       string    `orm:"column(tags);size(32);type(char);default();description(前缀标签)" json:"tags" form:"tags" validate:"required" label:"前缀标签"`
